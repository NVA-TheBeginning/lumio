...
describe('MicroserviceProxyService', () => {
        ...
        const module = await Test.createTestingModule({
            providers: [
                MicroserviceProxyService,
                {
                    ...
                    useValue: { get: (key: string) => 'http://test-service.com' },
                },
            ],
        }).compile();
    ...
    test('throws if microservice URL not configured', async () => {
        (configService.get as any) = () => undefined;

        await expect(
            service.forwardRequest('auth', '/foo', 'GET'),
        ).rejects.toThrow('URL du microservice "auth" non configurée.');
    });
    ...
    describe('forwardRequest / success cases', () => {
        ...
        beforeEach(() => {
            (configService.get as any) = () => baseUrl;
        });
        ...
        test('GET', async () => {
            ...
            const result = await service.forwardRequest<{ result: number }>(
                'auth',
                '/items',
                'GET',
                undefined,
        ...
        test('POST', async () => {
            ...
            const result = await service.forwardRequest<{ id: number }>(
                'auth',
                '/items',
                'POST',
                body,
        ...
        test('PUT', async () => {
            ...
            const result = await service.forwardRequest<{ name: string }>(
                'auth',
                '/items/1',
                'PUT',
                body,
        ...
        test('DELETE', async () => {
            ...
            const result = await service.forwardRequest<{ success: boolean }>(
                'auth',
                '/items/1',
                'DELETE',
            );
        ...
        test('PATCH', async () => {
            ...
            const result = await service.forwardRequest<{ active: boolean }>(
                'auth',
                '/items/1',
                'PATCH',
                body,
    ...
    describe('forwardRequest / error handling', () => {
        beforeEach(() => {
            (configService.get as any) = () => 'http://test-service.com';
        });
        ...
        test('transforms axios error into HttpException', async () => {
            ...
            await expect(
                service.forwardRequest('auth', '/fail', 'GET'),
